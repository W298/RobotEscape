            ...
            if (hits > 0)
            {
                for (int i = 0; i < hits; i++)
                {
                    ...
                    foreach (var sibling in siblings)
                    {
                        ...
                        {
                            if (RunningCoroutines.ContainsKey(sibling))
                            {
                                if (RunningCoroutines[sibling] != null)
                                {
                                    StopCoroutine(RunningCoroutines[sibling]);
                                }

                                RunningCoroutines.Remove(sibling);
                            }

                            RunningCoroutines.Add(sibling, StartCoroutine(FadeObjectOut(sibling)));
                            ObjectsBlockingView.Add(sibling);
                        }
                    }
                }
            }
